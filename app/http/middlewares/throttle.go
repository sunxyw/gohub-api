...
// ThrottleByIP 全局限流中间件，针对 IP 进行限流
...
//
func ThrottleByIP(limit string) gin.HandlerFunc {
	if app.IsTesting() {
		limit = "1000000-H"
	}
	...
	return func(c *gin.Context) {
		// 针对 IP 限流
		key := throttle.GetKeyIP(c)
		if ok := handleThrottle(c, key, limit); !ok {
			return
		}
		c.Next()
	}
...
// ThrottleByRoute 限流中间件，用在单独的路由中
func ThrottleByRoute(limit string) gin.HandlerFunc {
	if app.IsTesting() {
		limit = "1000000-H"
	}
	return func(c *gin.Context) {
		...
		// 针对 IP + 路由进行限流
		key := throttle.GetKeyRouteWithIP(c)
		if ok := handleThrottle(c, key, limit); !ok {
			return
		}
		c.Next()
...
func handleThrottle(c *gin.Context, key string, limit string) bool {
